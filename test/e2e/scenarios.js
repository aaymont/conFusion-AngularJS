...
describe('conFusion App E2E Testing', function() {
    ...
    describe('Aboutus', function() {
        ...
        it('should show Peter Pan as the first leader', function(){
            expect(element.all(by.repeater('leader in corpLeaders'))
                  .count()).toEqual(4);
        
            var leader = element.all(by.repeater('leader in corpLeaders')).first().element(by.binding('leader.name'));
            expect(leader.getText()).toContain('Peter Pan');
